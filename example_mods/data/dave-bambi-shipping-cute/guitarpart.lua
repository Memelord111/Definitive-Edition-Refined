...
function onStartCountdown()
 ...
 for i = 1, #a do
  ...
  if stringStartsWith(a[i], 'mods/scripts/icons') then
   removeLuaScript(a[i]);
  end
  if stringStartsWith(a[i], 'mods/scripts/DaveEngineHUD') then
   removeLuaScript(a[i]);
  end
  if stringStartsWith(a[i], 'mods/scripts/CoolCam') then
   removeLuaScript(a[i]);
  end    
 end 
